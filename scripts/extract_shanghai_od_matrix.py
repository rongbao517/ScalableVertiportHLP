# -*- coding: utf-8 -*-
"""
Build a (channel=1, T, 30, 30) origin-destination demand tensor for the
30 NSGA-selected Shanghai sites, in the format expected by the UAM_demand
ASTT model (`uam_airspace_context_od_v2.py`, copied unmodified into this
folder from /home/b5by/zhirong.b5by/UAM_demand/src_repo/model/).
...
Same data-quality handling as extract_shanghai_30min_demand.py:
`20150418.csv` is a byte-identical duplicate of `20150402.csv` (no real
2015-04-18 data) -> the file is skipped and the 48 bins for that day are
removed from the time axis entirely (compressed, not zero-filled), since
the ASTT training script assumes a contiguous time axis with no gaps.
29 real days * 48 bins/day = 1392 time steps.

Also builds a 30x30 directed adjacency (Gaussian kernel on haversine
distance between site centers) for use as the model's graph prior.
"""
...
OUT_NPZ = OUT_DIR / "shanghai_od_30min_1channel.npz"
OUT_ADJ = OUT_DIR / "shanghai_adj_matrix_30.csv"
OUT_META = OUT_DIR / "shanghai_od_meta.csv"
...
# full Shanghaidata_final.csv grid geometry (0.01-deg cells)
...
def main():
    ...
    # drop the missing day entirely (compress time axis, no gap-filling support in ASTT training script)
    ...
    arr_0 = od_compressed[np.newaxis].astype(np.float32)  # (channel=1, T, N, N)
