...
2015-04) for each of the 200 regenerated Stage-1 candidate sites
(predictions_shanghai_ranked_filtered_top200.csv, selected==1), directly
from Big paper/Shanghai raw taxi GPS trips.

Same method as compute_real_demand_candidates.py (used for the old 187
candidates); this is a separate output file so it does not overwrite the
existing candidate_real_demand.csv.
"""
...
RANK_CSV = DATA_DIR / "predictions_shanghai_ranked_filtered_top200.csv"
GRID_CSV = DATA_DIR / "Shanghaidata_final.csv"
OUT_CSV = BASE_DIR / "outputs" / "candidate_real_demand_top200.csv"
...
def main():
    ...
    grid = pd.read_csv(GRID_CSV, encoding="utf-8-sig").reset_index(drop=True)
