# -*- coding: utf-8 -*-
"""
Real site-to-site OD demand tensor for the 30 demand-weighted K-means sites
actually used by the current forecasting pipeline (../selected_sites_kmeans_K30.csv,
from run_site_selection_kmeans_K30.py -- the site set behind
shanghai_demand_windows_kmeans30.npz and the GRU/STID/TGCN training runs).

NOT the same as extract_shanghai_od_matrix.py's 30x30 tensor, which is built
for the older NSGA-selected selected_sites_K30.csv (different site set,
different column schema, no "Grid ID" column at all) -- mirrors
extract_shanghai_od_matrix_final10.py's approach instead, just with these
30 sites.
...
OUT_NPZ = OUT_DIR / "shanghai_od_kmeans30_30min_1channel.npz"
OUT_ADJ = OUT_DIR / "shanghai_adj_matrix_kmeans30.csv"
OUT_META = OUT_DIR / "shanghai_od_kmeans30_meta.csv"
...
def main():
    sites = load_sites()
    n_sites = len(sites)
    print(f"kmeans30 sites: {n_sites}")
    print(sites)
    ...
    arr_0 = od_compressed[np.newaxis].astype(np.float32)  # (channel=1, T, 30, 30)
