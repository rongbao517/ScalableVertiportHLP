...
structural sinks proactively -- before the hard cap is ever hit -- toward
vertiports that are structural net *sources* (bleeding vehicles out faster
than they're refilled), rather than just "whichever pile is currently
smallest" the way Tier 2 does.
"""
...
def redistribute_vehicles(vehicle_states, vertiport_states, distance_air, unmet_demand,
                           ...
                           predictive_min_history=None, debug=False):
    idle_by_vertiport = {}
    ...
    # toward persistent net-*source* vertiports before they ever trip the
    # hard idle cap. Opt-in: skipped entirely when flow_history is None, so
    # existing callers that don't pass it get the old Tier 1/2-only behavior.
    #
    ...
    # "positive" at any moment even with no structural sink at all), so Tier
    # 0 fired on ~150-200 vehicles/bin -- more of the fleet was shuttling
    ...
    if flow_history is not None:
        ...
        sinks = sorted(
            (v for v in all_vertiports
             if net_inflow[v] > predictive_threshold and len(idle_by_vertiport.get(v, [])) > min_reserve),
            key=lambda v: -net_inflow[v],
        )[:predictive_top_k]
        sources = [v for v in all_vertiports if net_inflow[v] < -predictive_threshold]

        for v in sinks:
            available = len(idle_by_vertiport.get(v, [])) - min_reserve
            if available <= 0:
                continue
            n_drain = min(available, predictive_max_drain, max(1, int(round(predictive_gain * net_inflow[v]))))

            candidate_targets = sources if sources else all_vertiports
    ...
    if max_idle_cap is not None:
        ...
        sources = sorted((v for v in all_vertiports if idle_counts[v] > max_idle_cap),
