...
(grid_o, grid_d) pairs -- proves the numpy shortcut (exact per-pair argmin,
valid because there's no capacity constraint coupling pairs together yet)
agrees with what a real solver returns, and gives a working Gurobi model
shape to extend once vertiport/corridor capacity constraints are added
(at which point pairs are no longer independent and this per-pair argmin
stops being valid -- a combined LP/MIP will be required).

Each sampled pair gets its own tiny model: 30x30=900 binary "take route
(i,j)" variables, one constraint (exactly one route chosen), objective =
minimize the same access+flight+egress time cost used in the main script.
...
ASSIGNMENT_CSV = OUT_DIR / "route_assignment_kmeans30.csv"

GROUND_SPEED_KMH = 25.0
AIR_SPEED_KMH = 200.0
DWELL_MIN = 5.0
N_SAMPLE = 20
SEED = 0
...
def solve_one_pair_gurobi(o_lat, o_lon, d_lat, d_lon, site_lat, site_lon):
    n = len(site_lat)
    access = haversine_km(o_lat, o_lon, site_lat, site_lon) / GROUND_SPEED_KMH * 60.0 + DWELL_MIN
    egress = haversine_km(d_lat, d_lon, site_lat, site_lon) / GROUND_SPEED_KMH * 60.0 + DWELL_MIN
...
def main():
    ...
    for k, idx in enumerate(sample_idx):
        ...
        gi, gj, gurobi_cost = solve_one_pair_gurobi(
            row["o_lat"], row["o_lon"], row["d_lat"], row["d_lon"], site_lat, site_lon
        )
        ...
        print(
            f"[{k+1}/{N_SAMPLE}] pair_idx={idx}  gurobi=(i={gi},j={gj},cost={gurobi_cost:.4f})  "
