...
def evaluate2(y_true, y_pred, Map):
    # print('MSE:', round(MSE(y_true, y_pred), precision))
    # print('RMSE:', round(RMSE(y_true, y_pred), precision))
    # print('MAE:', round(MAE(y_true, y_pred), precision))
    # print('MAPE:', round(MAPE(y_true, y_pred), precision), '%')
    # print('PCC:', round(PCC(y_true, y_pred), precision))
    return MSE2(y_true, y_pred, Map), RMSE2(y_true, y_pred, Map), MAE2(y_true, y_pred, Map), MAPE2(y_true, y_pred,Map)
...
def MSE2(y_true, y_pred, Map):
    ...
    with np.errstate(divide = 'ignore', invalid = 'ignore'):
        ...
        mask = mask.astype(np.float32)
        mask /= np.mean(mask)
        mse = np.square(y_pred - y_true)*Map
        mse = np.nan_to_num(mse * mask)
        mse = np.mean(mse)
        return mse
...
def RMSE2(y_true, y_pred, Map):
    ...
    with np.errstate(divide = 'ignore', invalid = 'ignore'):
        ...
        mask = mask.astype(np.float32)
        mask /= np.mean(mask)
        rmse = np.square(np.abs(y_pred - y_true))*Map
        rmse = np.nan_to_num(rmse * mask)
        rmse = np.sqrt(np.mean(rmse))
        return rmse
...
def MAPE2(y_true, y_pred, Map, null_val=0):
    # y_true[y_true < 20] = 0
    # y_pred[y_pred < 20] = 0
    y_true[y_true < 1e-5] = 0
    y_pred[y_pred < 1e-5] = 0
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        mask = mask.astype('float32')
        mask /= np.mean(mask)
        mape = np.abs(np.divide((y_pred - y_true).astype('float32'), y_true))*Map
        mape = np.nan_to_num(mask * mape)
        return np.mean(mape) * 100
